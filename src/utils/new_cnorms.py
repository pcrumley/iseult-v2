...
def PowerNormFunc(data, gamma=1.0, vmin=None, vmax=None, div_cmap=True,
    ...
    if div_cmap:
        pos_and_neg = np.sign(vmin) != np.sign(vmax)
        pos_and_neg = pos_and_neg and vmin != 0 and vmax != 0
        if pos_and_neg:
            x = [np.sign(vmin)*np.abs(vmin)**gamma,
                 0, np.sign(vmax)*np.abs(vmax)**gamma]
            y = [left_clip, 0.5, right_clip]
        elif np.sign(vmin) >= 0:
    ...
    if np.abs(midpoint) <= 1E-8 and np.abs(gamma-1.0) <= 1E-8:
        ...
        ans = np.ma.masked_array(
            np.interp(
                np.sign(data-midpoint)*np.abs(data-midpoint)**gamma, x, y))
    return ans
...
class SymLogNorm(colors.Normalize):
    ...
    idea is that it plots the log of the data, and stitches up around the
    midpoint, using a linear mapping.  If stretch_colors is true, then for
    diverging cmaps, the entire cmap is used, otherwise it only uses the
    amount so that -b and b are the same distance from the midpoint in
    color space.'''

    def __init__(self, vmin=None, vmax=None, clip=False, div_cmap=True,
                 midpoint=0.0, linthresh=1E-3, linscale=0.2,
                 stretch_colors=True):
        colors.Normalize.__init__(self, vmin, vmax, clip)
        self.div_cmap = div_cmap
    ...
    def __call__(self, value, clip=None):
        ...
        # First see if there is a sign change:
        ans = SymLogNormFunc(
            value, vmin=self.vmin, vmax=self.vmax, div_cmap=self.div_cmap,
            linthresh=self.thresh, linscale=self.linscale,
            stretch_colors=self.stretch_colors)
        if type(value) == np.ma.core.MaskedArray:
            ans.mask = value.mask
        return ans
...
def SymLogNormFunc(data,  vmin=None, vmax=None, div_cmap=True,  midpoint=0.0,
                   linthresh=1E-3, linscale=0.2, stretch_colors=True):
    ...
    if not stretch_colors:
        if np.sign(vmin) < 0 and np.sign(vmax) > 0:
            v_absmax = max(np.abs(vmin), np.abs(vmax))
            c1 = np.log10(v_absmax/linthresh)
            left_clip = (1-np.log10(np.abs(vmin/linthresh))/c1)
            left_clip *= 0.5*(1-linscale)
            right_clip = np.log10(np.abs(vmax)/linthresh)/c1
            right_clip *= 0.5*(1-linscale)
            right_clip += .5+.5*linscale

    ans = np.copy(data)
    left_log = np.where(data < midpoint - linthresh)
    #####
    ans[left_log] = np.interp(
        np.sign(data[left_log]-midpoint)
        * np.abs(np.log10(np.abs(data[left_log]-midpoint)/linthresh)),
        [np.sign(vmin)*np.log10(np.abs(vmin/linthresh)),
         np.sign(midpoint-linthresh)
         * np.log10(np.abs(midpoint-linthresh)/linthresh)],
        [left_clip, left_lin])

    # Now the linear middle
    linear_section = np.where(np.abs(data-midpoint) <= linthresh)
    ans[linear_section] = np.interp(
        data[linear_section] - midpoint,
        [midpoint - linthresh, midpoint + linthresh],
        [left_lin, right_lin])

    # Now the right part
    right_log = np.where(data >= midpoint + linthresh)
    ans[right_log] = np.interp(
        np.sign(data[right_log]-midpoint)
        * np.abs(np.log10(np.abs(data[right_log]-midpoint)/linthresh)),
        [np.sign(midpoint + linthresh)
         * np.log10(np.abs(midpoint+linthresh)/linthresh),
         np.sign(vmax)*np.log10(np.abs(vmax/linthresh))],
