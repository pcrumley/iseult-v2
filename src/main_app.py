...
#            self.iconify()
    ...
    def ReloadPath(self):
        ...
        get_flist_numbers.cache_clear()
        allFour = [elm for elm in get_flist_numbers(self.dirname)]

        if int(self.cmd_args.n)!=-1:
            self.CheckMaxNPopUp()

        if int(self.cmd_args.n) != -1 and len(allFour)>0:
            while (int(allFour[-1]) > int(self.cmd_args.n)) and len(allFour) > 0:
                allFour.pop(-1)

        self.TimeStep.setMax(len(allFour))
        self.playbackbar.slider.config(to =(len(allFour)))
        if self.MainParamDict['Reload2End']:
            self.TimeStep.value = len(allFour))
...
#                TotalKEDensity *= (self.DataDict['dens'][0,:,:].shape[0]/self.DataDict['c_omp'][0]*self.DataDict['istep'][0])**-1
    ...
    def shock_finder(self):
        ...
        '''
        DELETE THIS AFTER UPDATING FULLY!
